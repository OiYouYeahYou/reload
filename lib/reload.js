			...
			return new Promise(function (resolve, reject) {
				...
				if (socketPortSpecified) { // Use custom user specified port
					...
					if (httpsOption) { // HTTPS
						if (httpsOption.p12) {
							if (
								typeof httpsOption.p12.p12Path === 'string'
								&& httpsOption.p12.p12Path.match(/\.\w{3}$/)
							) {
								try {
									httpsOptions.pfx = fs.readFileSync(
										httpsOption.p12.p12Path
									)
								} catch (err) {
									return reject(err)
								}
